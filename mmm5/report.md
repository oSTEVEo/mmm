...
Функция правой части $f(x)$ получена подстановкой $u(x)$ в левую часть дифференциального уравнения, после упрощения:
$$
...
Аппроксимируем дифференциальный оператор $(p u')'$ следующим образом. Для равномерной сетки простейшая симметричная аппроксимация потока записывается непосредственно через значения $p$ в узлах:
$$
(p u')' \big|_{x_i} \approx \frac{1}{h}\left( p_{i+1}\frac{y_{i+1}-y_i}{h} - p_i\frac{y_i-y_{i-1}}{h} \right),
$$
где $p_i = p(x_i)$. Тогда разностное уравнение
$$
-\frac{p_{i+1}}{h^2}y_{i+1} + \frac{p_i+p_{i+1}}{h^2}y_i - \frac{p_i}{h^2}y_{i-1} + q_i y_i = f_i
$$
после умножения на $h^2$ принимает вид
$$
...
### 4. Свойства матрицы для метода наискорейшего спуска
...
Условие строгого диагонального преобладания проверено далее, см. раздел “Метод прогонки”
...
где:
- $r^k = b - Ay^k$ — вектор невязки на итерации $k$, определяющий направление спуска (направление наискорейшего убывания);
...
В нашем случае матрица $A$ трёхдиагональная с элементами:
- главная диагональ: $B_i = p_i + p_{i+1} + h^2 q_i$,
- нижняя диагональ: $A_i = -p_i$,
- верхняя диагональ: $C_i = -p_{i+1}$,

где $p_i = 1 + x_i^2$, $q_i = x_i + 1$, $x_i = i h$. Симметрия очевидна ($A_{i,i-1}=A_{i-1,i}$), а положительная определённость гарантирована тем, что разностная схема аппроксимирует положительно определённый эллиптический оператор $-\nabla(p\nabla) + q$ с $p>0$, $q>0$ при однородных граничных условиях Дирихле. Таким образом, условия сходимости выполнены.
...
где для $n=40$ задана точность $\epsilon = h^3 = \left(\frac{1}{40}\right)^3 = 0.000015625$.

### Метод прогонки
Для проверки будем сравнивать решение методом наискорейшего спуска с решением методом прогонки.  
...
$$ A_i y_{i-1} + B_i y_i + C_i y_{i+1} = D_i, \qquad i = 1,\dots,N \;(N=n-1), $$

где для нашей задачи:
$$ \begin{aligned}
A_i &= -p_i = -\big(1 + (ih)^2\big), \\
B_i &= p_i + p_{i+1} + h^2 q_i = \big(1 + (ih)^2\big) + \big(1 + ((i+1)h)^2\big) + h^2(ih+1), \\
C_i &= -p_{i+1} = -\big(1 + ((i+1)h)^2\big), \\
D_i &= f_i h^2 = \big(-x_i^5 + 20x_i^4 - 11x_i^3 + 12x_i^2 - 6x_i\big) h^2, \quad x_i = ih.
\end{aligned} $$

Метод состоит из двух проходов: прямого и обратного. Идея заключается в том, чтобы на прямом ходу выразить $y_i$ через $y_{i+1}$ с помощью вспомогательных коэффициентов:

$$ y_i = \alpha_{i+1} y_{i+1} + \beta_{i+1}. $$

#### Прямой ход
Идём слева направо (от $i=1$ до $N-1$).

Для первого узла ($i=1$, $y_0=0$):
$$ \alpha_{2} = -\frac{C_1}{B_1}, \qquad
   \beta_{2} = \frac{D_1}{B_1}. $$

Рекуррентные формулы для $i=2,\dots,N-1$:
$$ \alpha_{i+1} = -\frac{C_i}{A_i \alpha_i + B_i}, \qquad
   \beta_{i+1} = \frac{D_i - A_i \beta_i}{A_i \alpha_i + B_i}. $$

На последнем шаге при $i=N$ коэффициент $C_N$ отсутствует, и из уравнения
$A_N y_{N-1} + B_N y_N = D_N$ с подстановкой $y_{N-1} = \alpha_N y_N + \beta_N$ получаем:
$$ y_N = \frac{D_N - A_N \beta_N}{A_N \alpha_N + B_N} \equiv \beta_{N+1}. $$

#### Обратный ход
После определения $\beta_{N+1}$ (т.е. $y_N$) последовательно вычисляем остальные значения:
...
Подставляя выражения для $A_i, B_i, C_i$, получаем:
$$ |B_i| = p_i + p_{i+1} + h^2 q_i, \qquad |A_i| + |C_i| = p_i + p_{i+1}. $$
Так как $p(x) > 0$ и $q(x) = x+1 \ge 1 > 0$ на отрезке $[0,1]$, имеем $h^2 q_i > 0$, и, следовательно,
$$ |B_i| = |A_i| + |C_i| + h^2 q_i > |A_i| + |C_i| $$
для **каждого** уравнения. Таким образом, достаточное условие устойчивости прогонки выполнено.
